      ...
      <div className="flex flex-row">
        {loading ? (
          ...
          <div className="overflow-auto">
            {data.data.map((x) => {
              const activeChatColort =
                x.otherUserId === recieverId
                  ? "bg-gradient-to-r from-sky-200 to-slate-50"
                  : "";
              return (
                <div
                  key={Math.random()}
                  onClick={() => updateChat(x.otherUserId)}
                  className={`flex flex-row w-[400px] border border-grey rounded-lg p-2 m-1 shadow-sm hover:bg-gray-200 ${activeChatColort}`}
                >
                  <img
                    src={x.profileImage || "/samplePost.jpeg"}
                    className="w-[60px] h-[60px] rounded-full border-1 border-grey"
                    alt="post"
                  />
                  <div className="flex flex-col justify-center mx-[11px] text-lg">
                    ...
                    <p className="text-md">
                      ...
                      <span className="text-xs text-slate-600">
                        {moment(x.time * 1000).format("YYYY-MM-DD HH:mm")}
                      </span>
                    </p>
                  </div>
                </div>
